...
def test_mocked_status_code(requests_mock):
    ...
    requests_mock.get(f"https://api.eu.onfido.com/v3.6/applicants/{fake_uuid}", exc=requests.HTTPError(response=response))
    with pytest.raises(OnfidoRequestError):
        api.applicant.find(fake_uuid)

def test_onfido_unknown_error(requests_mock):
    requests_mock.get(f"https://api.eu.onfido.com/v3.6/applicants/{fake_uuid}", text="NOT VALID JSON")
...
def test_region_error():
    with pytest.raises(OnfidoRegionError):
        onfido.Api("<AN_API_TOKEN>", region="https://api.onfido.com/v3.6/")
