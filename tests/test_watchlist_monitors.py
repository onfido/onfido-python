...
def test_create_monitor(requests_mock):
    mock_create = requests_mock.post("https://api.eu.onfido.com/v3.6/watchlist_monitors/", json=[])
    api.watchlist_monitor.create(watchlist_details)
    assert mock_create.called is True

def test_find_monitor(requests_mock):
    mock_find = requests_mock.get(f"https://api.eu.onfido.com/v3.6/watchlist_monitors/{fake_uuid}", json=[])
    api.watchlist_monitor.find(fake_uuid)
    assert mock_find.called is True

def test_delete_monitor(requests_mock):
    mock_delete = requests_mock.delete(f"https://api.eu.onfido.com/v3.6/watchlist_monitors/{fake_uuid}", json=[])
    api.watchlist_monitor.delete(fake_uuid)
    assert mock_delete.called is True

def test_list_monitors(requests_mock):
    mock_list = requests_mock.get(f"https://api.eu.onfido.com/v3.6/watchlist_monitors/?applicant_id={fake_uuid}", json=[])
