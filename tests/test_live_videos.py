...
def test_find_live_video(requests_mock):
    mock_find = requests_mock.get(f"https://api.onfido.com/v3.1/live_videos/{fake_uuid}", json=[])
    api.live_video.find(fake_uuid)
    assert mock_find.called is True

def test_list_live_videos(requests_mock):
    mock_list = requests_mock.get(f"https://api.onfido.com/v3.1/live_videos/?applicant_id={fake_uuid}", json=[])
    api.live_video.all(fake_uuid)
    assert mock_list.called is True

def test_download_live_video(requests_mock):
    mock_download = requests_mock.get(f"https://api.onfido.com/v3.1/live_videos/{fake_uuid}/download", text="FAKE VIDEO BINARY", headers={"Content-type": "video/mp4"})
...
def test_download_live_video_frame(requests_mock):
    mock_download = requests_mock.get(f"https://api.onfido.com/v3.1/live_videos/{fake_uuid}/frame", text="FAKE VIDEO BINARY", headers={"Content-type": "video/mp4"})
