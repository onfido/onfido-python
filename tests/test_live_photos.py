...
def test_upload_photo(requests_mock):
    mock_upload = requests_mock.post("https://api.onfido.com/v3.1/live_photos/", json=[])
...
def test_find_live_photo(requests_mock):
    mock_find = requests_mock.get(f"https://api.onfido.com/v3.1/live_photos/{fake_uuid}", json=[])
    api.live_photo.find(fake_uuid)
    assert mock_find.called is True

def test_list_live_photos(requests_mock):
    mock_list = requests_mock.get(f"https://api.onfido.com/v3.1/live_photos/?applicant_id={fake_uuid}", json=[])
    api.live_photo.all(fake_uuid)
    assert mock_list.called is True

def test_download_live_photo(requests_mock):
    mock_download = requests_mock.get(f"https://api.onfido.com/v3.1/live_photos/{fake_uuid}/download", text="FAKE IMAGE BINARY", headers={"Content-type": "image/png"})
