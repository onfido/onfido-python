...
def test_upload_document(requests_mock):
    mock_upload = requests_mock.post("https://api.onfido.com/v3.1/documents/", json=[])
...
def test_find_document(requests_mock):
    mock_find = requests_mock.get(f"https://api.onfido.com/v3.1/documents/{fake_uuid}", json=[])
    api.document.find(fake_uuid)
    assert mock_find.called is True

def test_list_documents(requests_mock):
    mock_list = requests_mock.get(f"https://api.onfido.com/v3.1/documents?applicant_id={fake_uuid}", json=[])
    api.document.all(fake_uuid)
    assert mock_list.called is True

def test_download_document(requests_mock):
    mock_download = requests_mock.get(f"https://api.onfido.com/v3.1/documents/{fake_uuid}/download", text="FAKE IMAGE BINARY", headers={"Content-type": "image/png"})
