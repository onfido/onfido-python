...
def test_create_check(requests_mock):
    mock_create = requests_mock.post("https://api.eu.onfido.com/v3.4/checks/", json=[])
...
def test_find_check(requests_mock):
    mock_find = requests_mock.get(f"https://api.eu.onfido.com/v3.4/checks/{fake_uuid}", json=[])
    api.check.find(fake_uuid)
    assert mock_find.called is True

def test_list_checks(requests_mock):
    mock_list = requests_mock.get(f"https://api.eu.onfido.com/v3.4/checks/?applicant_id={fake_uuid}", json=[])
    api.check.all(fake_uuid)
    assert mock_list.called is True

def test_resume_check(requests_mock):
    mock_resume = requests_mock.post(f"https://api.eu.onfido.com/v3.4/checks/{fake_uuid}/resume", json=[])
    api.check.resume(fake_uuid)
    assert mock_resume.called is True

def test_download_check(requests_mock):
    mock_download = requests_mock.get(f"https://api.eu.onfido.com/v3.4/checks/{fake_uuid}/download", text="FAKE PDF BINARY", headers={"Content-type": "application/pdf"})
