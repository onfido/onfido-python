...
def test_find_report(requests_mock):
    mock_find = requests_mock.get(f"https://api.onfido.com/v3/reports/{fake_uuid}", json=[])
    api.report.find(fake_uuid)
    assert mock_find.called is True

def test_list_reports(requests_mock):
    mock_list = requests_mock.get(f"https://api.onfido.com/v3/reports/?check_id={fake_uuid}", json=[])
    api.report.all(fake_uuid)
    assert mock_list.called is True

def test_resume_report(requests_mock):
    mock_resume = requests_mock.post(f"https://api.onfido.com/v3/reports/{fake_uuid}/resume", json=[])
    api.report.resume(fake_uuid)
    assert mock_resume.called is True

def test_cancel_report(requests_mock):
    mock_cancel = requests_mock.post(f"https://api.onfido.com/v3/reports/{fake_uuid}/cancel", json=[])
